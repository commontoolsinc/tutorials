...
abstract: |
  In this section, we will create recipes to make LLM calls. We'll interate on them to make new features, making sure you understand the changes each step of the way.
---
(skeleton_recipe)=
## Skeleton Recipe
...
The [UI] property has all the information to render the charm in the browser. It contain a mix of JSX and interpolated function calls using `{some_function()}` syntax. The results of these functions are inserted into display for rendering. We'll learn more about what works in the UI property in future sections.
...
A `Cell` is like a variable in programming languages. They can store values and can be displayed in a recipe's [UI]. They are also persistent and automatically saved to the datastore.
...
We first imported the funciton `handler` from {ref}`imports`. It takes in type parameters for the Event it will handle and the Args it will be passed (`handler<Event, Args>`). The shape of `Event` is based on the component you use. In our case, we used the component `common-send-message`. It takes in an event with the shape `{detail: {message: string}}`, you see this reflected on line 2. The shape of `Args` depend on what we are passing into the handler. We've set this in {ref}`llm_code_03` line 15, where we pass in `userMessage`, which is a Cell. We therefore set the Args type as `{ userMessage: Cell<string> }` (line 3).
...
The next step is the real pay-off. We'll finally call the LLM. We'll add the built-in call right after the userMessage definition in the recipe. The actual location doesnt matter as long as it's in the `recipe`'s body.
```{code-block} typescript
:lable: llm_builtin
:linenos: true
:elphasize-lines:
...
This gets a bit technical. The parameters to a built-in like `llm()` as passed as an `Opaque` object. Opaque objects mirror the passed in type, but any of the properties that are `Cell`s become reactive. We want to have the `messages` property be reactive, meaning that it will be updated anytime the `userMessage` changes. There are a few ways to add in the Cell here, but the easiest is to use `derive` which will create a reactive node.
...
We'll add a new section to the recipe [UI] to display the current value of the `llmResponse`. Luckily, this is very straight-forward:
```{code-block} typescript
:lable: llm_response
:linenos: true
:elphasize-lines:
...
```
`llmResponse` is typed as `LLMBuiltInState`. The important thing to know about this is that it has a `result?` property, which, not surprisingly, as the result of the llm call.
...
There's a lot more to discover with the llm() function call (such as sending a list of user and agent message for history or even tool use) and even more to learn about the Common Tools runtime system.
